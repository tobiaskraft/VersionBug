...
class PersonController {
     ...
     * ###################################################################
     * ###################################################################
     * The problem seems to be that the method argument person has not
     * assigned the value of the version property from the gsp file.
     * It seems that the version property is overwritten with the value from the database
     * which is wrong in this case.
     * In the params object the version value is correct.
     *
     * See also the logging message at the beginning of the method.
     *
     * ###################################################################
     * ###################################################################
     */
    def update(Person person) {
        log.error("domain person ${person.firstName} ${person.lastName} has version ${person.version} and " +
                "the param parameter has the value ${params.version}")
